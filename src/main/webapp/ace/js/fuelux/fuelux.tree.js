...
(function (factory) {
    ...
    var Tree = function (element, options) {
        ...
        this.$element.on('click.fu.tree', '.tree-branch-header', $.proxy(function (ev) {
            if (options.singleOpen === true) {
                var self = this;

                // 已展开
                if(!$(ev.currentTarget).parent().is('.tree-open')) {
                    this.openFolder(ev.currentTarget);
                }

                this.$element.find('.tree-open .tree-branch-header').not(ev.currentTarget).each(function (i, el) {
                    //self.openFolder(el);
                });
            } else {
                this.openFolder(ev.currentTarget);
